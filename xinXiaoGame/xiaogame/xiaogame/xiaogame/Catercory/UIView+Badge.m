...
- (void)layoutBadge:(UIView *)view
{
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeTrailing multiplier:1 constant:-7]];
...
- (void)setBadge:(void(^)(BadgeBuilder *builder))buildHandler;
{
    ...
    if (builder.number) {
        ...
        self.badgeLabel.font = builder.font ?: [UIFont systemFontOfSize:W(9)];
        self.badgeLabel.size = builder.size;
        self.badgeLabel.text = builder.number.integerValue == 0 ? @"0" : ((builder.number.integerValue > 99) ? @"99+" : builder.number.stringValue);
